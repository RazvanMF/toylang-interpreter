...
public class CountdownLatchStatement implements IStatement {
    ...
    public CountdownLatchStatement(String latchKey) {
    ...
    public ProgramState execute(ProgramState state) throws StandardException, IOException {
        ...
        if (latchKeyLookup != null) {
            ...
            if (latchTable.isDefined(latchAddr)) {
                ...
                int ID = state.getPersonalID();
                if (remaining > 0) {
                    latchTable.update(latchAddr, remaining - 1);
                    out.add(new StringValue(String.format("Program State ID %s invoked countdown of Latch %s",
                            ID, latchAddr)));
                }
    ...
    public String toString() {
        return "Countdown on Latch (" + latchKey + ")";
    }
}
